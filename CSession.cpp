...
void CSession::close() {
	...
	if (out.is_open()) {
		for(const auto & p: msg_pool)
			out << p.first << '|' << p.second->get_sender() << '|' << p.second->get_recv() << '|' << p.second->get_body() << '|' << p.second->get_type() << std::endl;
	}
	out.close();
}
//--------------------------------------------------------------------------------------
...
void CSession::save_user_data() const {
	...
	if (current_user)
		out << current_user->get_name() << ',' << hash_to_string(current_user->get_pwd_hash()) << std::endl;
